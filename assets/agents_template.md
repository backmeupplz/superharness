...
Run `$HARNESS models` to see available models. Always use `--model` when spawning workers.
Run `$HARNESS auth list` to see authenticated providers. Only use models from authenticated providers.
...
3. For tasks with `status: "in-progress"` at startup — workers likely crashed. Verify with `$BIN list`; respawn or reset to `pending`.
...
> **When done**: run `$BIN heartbeat`

After worker finishes: `git merge <branch>` from main repo, then `$BIN kill --pane %ID` (auto-cleans worktree). Never have two workers editing the same file simultaneously.

## Approving Worker Actions

- **APPROVE** safe ops (edits, reads, git, builds, tests): `$BIN send --pane %ID --text "y"`
...
When `$BIN read` shows a worker is done:
1. Read final output
2. `git merge <worker-branch>` (from main repo)
3. `$BIN kill --pane %ID` (auto-cleans worktree)
4. Set task `done` in `tasks.json`, clear `worker_pane`
5. `$BIN run-pending` to unblock dependent tasks
...
```bash
$BIN spawn --task "fix race condition in auth" --name "fix-race" --dir /repo --model anthropic/claude-sonnet-4-6
$BIN spawn --task "fix typo in error message" --name "fix-typo" --dir /repo --model anthropic/claude-haiku-4-5
```
...
After each kill: run `$BIN run-pending` to auto-spawn unblocked tasks.

## Harness Management

Current: **$HARNESS_DISPLAY**. `$BIN harness-list` / `$BIN harness-set <name>` / F2 popup. Use `--harness <name>` per worker. When user says "use codex/claude": run `$BIN harness-set <name>` immediately.

Workers cannot spawn sub-workers (enforced). Break large tasks into scoped units and spawn from superharness.

## Worker Failure Recovery

Crashed/stuck: `$BIN respawn --pane %ID --task "..." --dir /path --model $DEFAULT_MODEL`
Needs nudge: `$BIN send --pane %ID --text "..."`

$PREFERENCES

## Model Selection

Match model to task complexity — do not always default to `$DEFAULT_MODEL`.
